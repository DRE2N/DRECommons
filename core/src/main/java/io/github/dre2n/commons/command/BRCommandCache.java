...
/**
 * Note that DRE2N plugins are usually designed to have just one instance of BRCommandCache.
 * One instance of BRCommandCache represents one command and contains all of its subcommands.
 *
 * @author Daniel Saukel
 */
public class BRCommandCache {

    private String label;
    private CommandExecutor executor;
    private Set<BRCommand> commands = new HashSet<>();

    public BRCommandCache(String label, BRPlugin plugin, Set<BRCommand> commands) {
        this.label = label;
        this.executor = new BRCommandExecutor(plugin);
        this.commands = commands;
    }

    public BRCommandCache(String label, BRPlugin plugin, BRCommand... command) {
        this.label = label;
        this.executor = new BRCommandExecutor(plugin);
        this.commands = new HashSet<>(Arrays.asList(command));
    }

    public BRCommandCache(String label, CommandExecutor executor, Set<BRCommand> commands) {
        this.label = label;
        this.executor = executor;
        this.commands = commands;
    }

    public BRCommandCache(String label, CommandExecutor executor, BRCommand... command) {
        this.label = label;
        this.executor = executor;
        this.commands = new HashSet<>(Arrays.asList(command));
    }
